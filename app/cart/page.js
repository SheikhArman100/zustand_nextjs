...
const Cart = () => {
  ...
  const totalPrice =useMemo(() => cart?.filter((product) => product.addToCheckout).reduce((sum, product) => sum + parseInt(product.totalPrice), 0), [cart]) 
                ...
                <tr className="border-gray-200" key={item.id}>
                  ...
                  <td className="flex items-center ">
                    ...
                    <div className="btn-group items-center gap-x-1">
                      <button
                        className="btn btn-sm bg-transparent text-black border-gray-300 hover:text-white"
                        onClick={() => decreaseQuantity(item.id)}
                      >
                        -
                      </button>
                      <p className="leading-[2rem] bg-base-300 px-6 bg-transparent border ">
                        {item.quantity}
                      </p>
                      <button
                        className="btn btn-sm bg-transparent text-black border-gray-300 hover:text-white"
